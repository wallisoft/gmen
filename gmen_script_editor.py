#!/usr/bin/env python3
"""
GMen Script Editor v4.0 - Multi-language script editor with Lua support
...
from core.scripting.engine import ScriptEngine
...
class GMenScriptEditor:
    def __init__(self, script_id=None):
        ...
        self.script_engine = ScriptEngine(config_dir)
    ...
    def create_toolbar(self):
        ...
        # Scripts List button
        list_btn = Gtk.Button(label="📋 Scripts List")
        list_btn.connect("clicked", self.show_scripts_list)
        ...
        hbox.pack_start(list_btn, False, False, 0)
    ...
    def setup_line_numbers(self):
        ...
        def update_line_numbers(*args):
            ...
            self.status_label.set_text(f"Lines: {line_count} | Characters: {char_count}")
    ...
    def on_run(self, button):
        ...
        try:
            if language == 'lua':
                result = self.script_engine.execute_lua(content, {})
            elif language == 'python':
                result = self.script_engine.execute_python(content, {})
            else:  # shell
                result = self.script_engine.execute_shell(content, {})
...
function main()
    -- Launch VS Code
    local code_pid = gmen.launch("code")
    gmen.set_window(code_pid, 100, 100, 1200, 800)
    
    -- Launch terminal
    gmen.sleep(1)
    local term_pid = gmen.launch("gnome-terminal")
    gmen.set_window(term_pid, 1300, 100, 600, 800)
...
"""
    ...
    def show_scripts_list(self, button):
        ...
        for script in scripts:
            ...
            # Script icon based on language
            icon_map = {'lua': '📜', 'python': '🐍', 'shell': '💻'}
            icon = icon_map.get(script['language'].lower(), '📝')
            
            label = Gtk.Label(label=f"{icon} {script['name']} ({script['language']})")
            label.set_xalign(0)
            hbox.pack_start(label, True, True, 0)
            
            # Load button
            load_btn = Gtk.Button(label="📂")
            load_btn.set_tooltip_text("Load this script")
            load_btn.connect("clicked", lambda w, s=script: self.load_script_from_list(s['id'], dialog))
    ...
    def load_script_from_list(self, script_id, dialog):
        """Load a script from the list"""
        dialog.destroy()
